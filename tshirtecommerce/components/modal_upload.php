					...
					<div class="tab-pane" id="uploaded-art">
						<div class="row" id="dag-files-images">
							...
							if(isset($_COOKIE['user_files'])) {
								$user_files = json_decode($_COOKIE['user_files'], true);
								foreach ($user_files as $file_key => $file)
								{
									echo '<div class="view-thumb col-xs-4 col-md-4" id="file-'.$file_key.'" onclick="design.myart.loadFile(\''.$file_key.'\')">';
									echo '<img class="img-responsive img-thumbnail" src="'.$file['thumb'].'" alt="">';
									echo '</div>';
								}
							}
							?>
