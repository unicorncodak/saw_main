...
{
	$temp = explode('//www.', $site_url);
	if (count($temp) == 1)
	{
		$site_url = str_replace('http://', 'http://www.', $site_url);
	}
}
else
{
...
<script type="text/javascript">
	function pagination(segment, page)
	{
		...
		jQuery.ajax({
			type: "GET",
			url: '<?php echo $site_url; ?>tshirtecommerce/admin-create.php?/'+segment+'&page='+page+'&search='+search,
			dataType: 'html',
			beforeSend: function(){
