		...
		if (isset($product))
		{
			...
			if (count($design))
			{
				...
				if (isset($area->$position) && $area->$position != '')
				{
					...
					$svg 		= '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" data-zoom="'.$zoom.'" height="'. $view->height .'" width="'.$view->width.'" data-main="1">';
					$svgPNG 	= '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" data-zoom="'.$zoom.'" height="'. $view->height .'" width="'.$view->width.'" data-main="1">';
					...
					if(isset($radius) && $radius > 0)
					{
						...
						else
						{
							...
							$svgPNG	.= '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="'.($view->width).'" height="'.($view->height).'">'
										.'<rect x="0" y="0" rx="'.($radius*$zoom).'" ry="'.($radius*$zoom).'" width="'.($view->width*$zoom).'" height="'.($view->height*$zoom).'" style="fill:none;stroke:#FFFFFF;stroke-width:5"/>'
										.'</svg>';
						}
					}
