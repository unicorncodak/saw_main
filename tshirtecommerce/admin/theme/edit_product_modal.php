	...
	<form id="fr-product" accept-charset="utf-8" method="post" action="<?php echo site_url('index.php/product/save'); ?>">
		<div class="tabbable col-md-12">
			<ul id="myTab" class="nav nav-tabs tab-bricky">
				<li class="active">
					<a href="#panel_tab2_example1" data-toggle="tab" aria-expanded="true">
						<i class="green fa fa-home"></i> <?php lang('product_info'); ?>
					</a>
				</li>
				<li>
					<a href="#panel_tab2_example2" data-toggle="tab" aria-expanded="true">
			...
			<div class="tab-content">
				<div class="tab-pane active" id="panel_tab2_example1">
					...
					<div class="panel panel-default">
						<div class="panel-heading">
							<i class="clip-data"></i> <?php lang('product_data'); ?>
							<div class="panel-tools">
								<a href="javascript:void(0);" class="btn btn-xs btn-link panel-collapse collapses"></a>										
							</div>
						</div>
						...
						<div class="panel-body">
							<div class="tabbable">
								<ul class="nav nav-tabs tab-bricky">
									...
									<li><a href="#tabs-2" data-toggle="tab" aria-expanded="true"><i class="green clip-cog"></i> Advanced</a></li>
								</ul>
								<div class="tab-content">
									<div id="tabs-1" class="tab-pane active">
										...
										<div class="clear-line" style="display: none;"></div>																			
										<div class="form-group" id="prices-quantity" style="display: none;">
...
<script type="text/javascript">
	...
	function productInfo2(product) {
		jQuery('#product-name,input[name="product[title]"]').val(product.title);
		jQuery('.short_description,input[name="product[description]"]').val(product.shortdescription);
		jQuery('.product_description,input[name="product[short_description]"]').val(product.description);
		jQuery('.product_sku,input[name="product[sku]"]').val(product.sku);
		jQuery('.product_price,input[name="product[price]"]').val(product.price);
		jQuery('.product_sale_price,input[name="product[sale_price]"]').val(product.sale_price);
		jQuery('#products_image,input[name="product[image]"]').val(product.thumb);
		jQuery('#product_min_order,input[name="product[min_order]"]').val(product.min_order);
		jQuery('#product_max_order,input[name="product[max_oder]"]').val(product.max_order);
		...
		if (product_js.saveProduct() == true) {
			...
			jQuery.ajax({
				data: data_form,
				type: 'post',
				url: site_url+'/index.php?/oproduct/ocsave',
				dataType:'json',
				async: false,
