...
class Settings extends Controllers
{
	...
	public function publishColor()
	{
		...
		foreach($_POST['checkb'] as $id)
		{
			...
			{
				$colors['colors'][$id] = array(
						'title'=>$colors['colors'][$id]['title'],
						'hex'=>$colors['colors'][$id]['hex'],
						'published'=>1,
					);
			}
		}
	...
	public function unpublishColor()
	{
		include_once(ROOT.DS.'includes'.DS.'functions.php');
		$dg = new dg();
	...
	public function editLanguage($id = '')
	{
		...
		if(isset($_POST['data']))
		{
			...
			if($id === '')
			{
				$post['default'] = 0;
				$languages[] = $post;
				$new_file = dirname(ROOT).DS.'data'.DS.$post['file'];
				/* read language */
				$file_lang 		= dirname(ROOT).DS.'data'.DS.'lang.ini';
				$lang 		= file_get_contents($file_lang);

				$file_lang_plus 	= dirname(ROOT).DS.'data'.DS.'lang_plus.ini';
				$lang_plus 		= file_get_contents($file_lang_plus);
...
'.$lang_plus;
	...
	function getFont()
	{
		include_once(ROOT.DS.'includes'.DS.'functions.php');
		$dg = new dg();
		echo $dg->readFile(dirname(ROOT).DS.'data'.DS.'googlefonts.json');
		exit;
	}
}
