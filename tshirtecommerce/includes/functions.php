...
class dg{
	...
	public function view($name, $path = '')
	{
		...
		if(isset($this->product->theme) && $this->product->theme != '')
		{
			...
			if( $layouts != false && count($layouts) && isset($layouts[$this->product->theme]) )
			{
				...
				$theme 			= $layout['theme'];
				$new_file 		= ROOT .DS. 'themes' .DS. $layout['theme'] .DS. 'components' .DS. $name. '.php';
				if (file_exists($new_file))
				{
					$file = $new_file;
				}
	...
	public function prices($data, $add_tax = true)
	{
		...
		if ($product === false)
		{
			...
			//exit;
			if (!isset($total)) $total = new stdClass();
			$total->item 		= 0;
			$total->printing 	= 0;
			$total->clipart 	= 0;
			$total->old 		= 0;
		...
		else
		{
			...
			$total->printing 	= $cart->priceFormart($result->price->prints) - $print_discount;
			$total->sale 		= $cart->priceFormart($result->price->sale) + $cart->priceFormart($result->price->colors) + $cart->priceFormart($total->printing);
			...
			if (isset($data['product_id_oc']) || isset($data['_product_id_oc'])) {
