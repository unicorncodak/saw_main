...
var tshirtIntroduction = {
	...
	show       : function(introEle) {
		...
		jQuery('.intro-desc-taskbar .close-desc').click(function() {
			...
			if(tshirtIntroduction.isMobie = true)
			{
				jQuery('#dg-left .dg-box').animate({ scrollLeft: jQuery('#dg-left .dg-box').prop("scrollWidth")}, 0);
				jQuery('#dg-left .dg-box').animate({ scrollLeft: 0}, 0);
				jQuery('.dg-tools .list-tools').animate({ scrollLeft: jQuery('.dg-tools .list-tools').prop("scrollWidth")}, 0);
				jQuery('.dg-tools .list-tools').animate({ scrollLeft: 0}, 0);
			}
		});
		...
		/** Create effect show */
		setTimeout(function() {
			jQuery('.introduction-ele-mask').show(0);
		}, 1);
