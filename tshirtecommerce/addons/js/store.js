design.store = {
	...
	quick_edit:{
		...
		addPhoto: function(div, items){
			...
			{
				var item = items[i];
				var html = '<div class="custom-row custom-row-photo">'
					+ 	 	'<div class="custom-image">'
					+ 	 		'<img onclick="design.store.quick_edit.changePhoto(this, '+item.id+')" src="'+item.thumb+'" width="90" alt="">'
					+ 	 	'</div>'
					+ 	 	'<div class="custom-action">'
					+ 	 		'<a onclick="design.store.quick_edit.changePhoto(this, '+item.id+')" href="javascript:void(0);" title="">'+lang.store.quick_view.change+'</a>'
					+ 	 	'</div>'
					+  '</div>';
				div.append(html);
			}
