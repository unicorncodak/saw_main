	...
	item:{
		...
		imports: function(item){
			...
			if (item.type == 'clipart' && typeof item.file != 'undefined' && typeof item.file.type != 'undefined' && item.file.type == 'svg')
			{
				...
				jQuery(svg[0]).find('*').each(function() {
					var node = this.nodeName;
					if(node != 'defs' && node != 'style' && node != 'metadata')
					{
						var clss = jQuery(this).attr('class');
						if(clss != '' && clss != undefined)
						{
							clssArr = clss.split(' ');
							jQuery(clssArr).each(function() {
								if(this.indexOf('item-') == 0)
								{
									var indexEle = clssArr.indexOf(this);
									clssArr.splice(indexEle, 1);
								}
							});
							if(clssArr.length == 0)
							{
							...
							{
								jQuery(this).attr('class', clssArr.join(' '));
							}
						}
					}
				});
