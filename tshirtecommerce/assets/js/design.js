...
var design={
	...
	ini:function(){
		...
		$jd('.add_item_clipart').click(function(){
			if(jQuery('#dag-store-categories').length == 0)
			{
	...
	products:{
		...
		changeColor: function(e, n)
		{
			...
			if( jQuery('.product-design .is_change_color').length )
			{
				jQuery('.product-design .is_change_color').css('background-color', '#'+hex);
			}
				...
				if (typeof obj[i] != 'undefined')
				{
					...
					if (layer.id != 'area-design')
					{
						...
						imageObj.onload = function(){
							context.save();
							if(typeof layer.is_change_color != 'undefined' && layer.is_change_color == 1)
							{
								...
								temp_context.rect(0, 0, imageObj.width, imageObj.height);
							      temp_context.fillStyle = '#'+hex_color;
							      temp_context.fill();
