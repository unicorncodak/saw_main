...
if (strpos($_SERVER['HTTP_HOST'], 'www.') !== false) {
	...
	if (count($temp) == 1) {
		if (strpos($settings->site_url, 'https://') !== false) {
			$settings->site_url = str_replace('https://', 'https://www.', $settings->site_url);
		} else {
...
<head>
	...
	<link type="text/css" href="assets/css/tshirtecommerce.min.css" rel="stylesheet" media="all"/>
	<link type="text/css" href="themes/default/fonts/flaticon.css" rel="stylesheet" media="all">
	<link type="text/css" href="assets/css/style-mobile.css" rel="stylesheet" media="all">
	<?php echo $addons->css(); ?>
	
	<script type="text/javascript" src="assets/js/tshirtecommerce.min.js"></script>
...
jQuery("div.dg-box").scroll(function() {
    var documentScrollLeft = jQuery("div.dg-box").scrollLeft();
	...
	<script type="text/javascript">
		...
		$(function() {
    $('#dg-left').scroll( function() {
        /*var $width = $('#dg-left').outerWidth()
        var $scrollWidth = $('#dg-left')[0].scrollWidth; 
        var $scrollLeft = $('#dg-left').scrollLeft();
