...
switch($type){
	case 'upload':
		...
		{			
			...
			if($uploader->uploadFile('myfile'))
			{
				...
				setcookie('user_files', json_encode($user_files), time() + (86400 * 100) );
	...
	case 'loadaddonjs':
		...
		if(isset($settings->minifyjsdata) && count($settings->minifyjsdata) > 0)
		{
			...
			foreach($files as $key=>$val)
			{
				...
				{
					$jss 	.= $dg->readFile($path . $val.'.js'). PHP_EOL;
				}
			}
		}
		echo $jss;
